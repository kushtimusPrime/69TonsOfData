...
public class Player {
	  	...
	  	for (int x=0;x<3;x++)
	  	{
	        ...
	        while (true) {
	           ...
	           for (int i = 0; i < gc.myUnits().size(); i++)
	        	   ...
	        	   if (gc.myUnits().get(i).unitType() == UnitType.Healer)
	        	   {
	        		   healers.add(gc.myUnits().get(i));
	        	   }
	 ...
	 public static void KnightAttack(GameController gc, ArrayList<Unit> knights)
	 {
		 for (int x=0; x<knights.size();x++)
				  {
		 if (gc.canAttack(knights.get(x).id(), searchForAttack().id()) && gc.isAttackReady(knights.get(x).id()))
		 {
			 if (gc.canJavelin(knights.get(x).id(), searchForAttack().id()) && gc.isJavelinReady(knights.get(x).id())))
			 {
				 gc.javelin(knights.get(x).id(), searchForAttack().id())
			 }
			 else 
			 {
				 gc.attack(knights.get(x).id(), searchForAttack().id())
			 }
		 }
		 }
	 }
